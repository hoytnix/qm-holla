...
* **Step 4:** **BUILD & ASYNC POLLING RULE**: When running `pnpm build`, configure `WaitMsBeforeAsync: 10000` (10 seconds). Because `pnpm build` is an intensive ~30-second process, NEVER poll task status in tight 1-second loops; only inspect status every 10 seconds or await reactive task completion notifications.
